...
cd test_dft_open
./cm.test.sh $PREP $OUT out_true.o REFERENCE_fort.10_new $ROUND_OFF
exit_code=$?
...
cd test_vmc
./cm.test1.sh $TURBORVB $READF $FORCEVMC $OUT $OUT_FORCEVMC out_true.o out_true_forcevmc.o REFERENCE_fortXXI forces_REFERENCE $ROUND_OFF
exit_code=$?
exit_scores_arr+=($exit_code)
if [ $exit_code -ne 0 ]; then echo " Warn: Job failed."; fi
