...
program erread
    ...
    integer(4) k, kt, i, iread, j, ng, nrest, nbin, npm         &
            ...
            &, ntime, countmax, iflag, iflag_ave, rest, ikp, krep, optbra, nsite, nwr
    real(8) wsk, count_mes, tbra, etry, gamma, pip, pip1, pip0
    ...
    character(20) :: str
    logical :: yes_read, yes_weight, nodiv
...
101 if (iflag .eq. 1) kp_read = 1
    ...
    open (unit=11, file='parminimized.d', form='formatted', status='old')
    read (11, *)
    read (11, *) nwr, optbra, gamma, tbra, etry
    nodiv = .false.
    if (optbra .eq. -1) then
        rewind (11)
        read (11, *)
        read (11, *) nwr, optbra, gamma, tbra, etry, nsite
    elseif (optbra .eq. -2) then
        nodiv = .true.
        rewind (11)
        read (11, *)
        read (11, *) nwr, optbra, gamma, tbra, etry, nsite
    ...
    if (iskipr .ge. 0 .or. iflag .eq. 0) then
            ...
            if (kave_read .eq. 1) then
                do ikp = 1, nkps
                    if (ikp .eq. kp_read) then
                        if (krep .eq. 1) then
                            if (iskip .gt. 0) then
                                read (12, end=109, err=109) wbuf(j), pip, wtot(j), (eskip(k), k=1, iskip), ebuf(j, 1)
                            else
                                read (12, end=109, err=109) wbuf(j), pip0, pip1
                                ! growth estimate
                                wtot(j) = 1.d0
                                if (iskip .eq. 0) then
                                    ebuf(j, 1) = wbuf(j)
                            ...
                            read (12, end=109, err=109) wbuf(j), pip, wtot(j), eskip(1), (ebuf(j, k), k=1, iskip)
                            countread = countread + 1
                        end if
                        if (yes_weight) then
                            wbuf(j) = 1.d0
                ...
                do ikp = 1, nkps
                    if (ikp .ge. kp_read .and. ikp .lt. kp_read + kave_read) then
                        if (krep .eq. 1) then
                            if (iskip .ne. 0) then
                                read (12, end=109, err=109) wbuf(j), pip, wtot(j), (eskip(k), k=1, iskip), cost_mes(1)
                            else
                                read (12, end=109, err=109) wbuf(j), pip, wtot(j), cost_mes(1)
                            end if
                        else
                            read (12, end=109, err=109) wbuf(j), pip, wtot(j), eskip(1), (cost_mes(k), k=1, krep)
                        end if
...
109     continue
    ...
    if (iskip .eq. 0 .and. tbra .ne. 0.d0) then
        write (20, *) ' Growth estimate (H)  error, # of bias  correcting factor '
        if (optbra .eq. -1) then
            do i = 0, maxk
                write (20, *) (etry - dlog(gapjack(i + 1, 1))/tbra)/nsite, &
                        &abs(gaps(i + 1, 1)/gapjack(i + 1, 1))/tbra/nsite, i
            end do
        else
            do i = 0, maxk
                write (20, *) etry - dlog(gapjack(i + 1, 1))/tbra, abs(gaps(i + 1, 1)/gapjack(i + 1, 1))/tbra, i
            end do
        end if
    else
