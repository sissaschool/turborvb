...
module Spin2
    ...
    use logger_io, only: log_debug, log_info, log_warning, log_error    
...
contains
    ...
    subroutine compute_spin2(kel, Ainv, winv, winvj, winvbar, winvjbar, winvjbarsz, detmat, projm, mu_c, nmol)
        ...
        block
          ...
          do ii=1,ipf*ipc*nelorb
             do jj=1,nelup
                call log_debug(winbar(ii,jj))
             end do
          end do
          ...
          do ii=1,ipf*ipc*nelorb
             do jj=1,nelup
                call log_debug(winvbar_ext(ii,jj))
             end do
          end do
                ...
                if (ipf .eq. 2) then
                    ...
                    if (ipc .eq. 2) then
                        ...
                        block
                          ...
                          do jj=1,2
                             do ii=1,ipc*nelup
                                call log_debug(det_update_vec1(ii,jj))
                             end do
                          end do
                          ...
                          do jj=1,2
                             do ii=1,ipc*nelup
                                call log_debug(det_update_vec2(ii,jj))
                             end do
                          end do
                        end block
#endif
...
!                   call test_jastrow_spin_flip(JasSpin, k, l + nelup)
                ...
                if (.not. allocated(ratiospin)) call log_error("DEBUG HERE IT IS THE BUG")
