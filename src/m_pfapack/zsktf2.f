...
!    $                 A( 1, K-1 ), 1, A( 1, 1 ), LDA )
               CALL ZSKR2K(UPLO,'N',K-2,1,ONE/A( K-1,K ),A(1,K),LDA,
     $         A(1,K-1),LDA,ONE,A,LDA)
...
!    $                 A( K+2, K+2 ), LDA )
              CALL ZSKR2K(UPLO,'N',N-K-1,1,ONE/A( K+1,K ),A(K+2,K),LDA,
     $         A(K+2,K+1),LDA,ONE,A(K+2,K+2),LDA)
*     Store L(k+1) in A(k)
