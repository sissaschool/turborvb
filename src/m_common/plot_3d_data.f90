...
subroutine plot_3d_data(ipc, cell, cellscale, natoms, pos, zeta, periodic, mesh, origin, datagrid, indmol, word)
    ...
    double precision :: cell(3), cell_rescaled(3), cellscale(3), cellscale_rescaled(3)
    ...
    cell_rescaled = cell*length_unit
    cellscale_rescaled = cellscale*length_unit
    ...
    write (ofile, *) 'CRYSTAL '
    write (ofile, *) 'PRIMVEC '
    write (ofile, *) cellscale_rescaled(1), 0.0, 0.0
    write (ofile, *) 0.0, cellscale_rescaled(2), 0.0
    write (ofile, *) 0.0, 0.0, cellscale_rescaled(3)
    ...
    if (any(mesh(:) .ne. 0)) then
        ...
        write (ofile, '(3f14.8)') cell_rescaled(1), 0.0, 0.0
        write (ofile, '(3f14.8)') 0.0, cell_rescaled(2), 0.0
        write (ofile, '(3f14.8)') 0.0, 0.0, cell_rescaled(3)
        !
        !  do ix=1,mesh(1)
        !    do iy=1,mesh(2)
